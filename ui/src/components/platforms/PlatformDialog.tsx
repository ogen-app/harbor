...
const CONSTRAINT_TABS = ["Image", "Video", "PDF", "Text"] as const;
type ConstraintTab = (typeof CONSTRAINT_TABS)[number];
...
function PlatformForm({
  ...
  const [tab, setTab] = useState<ConstraintTab>("Image");
  ...
  const addPostType = () =>
    setPostTypes((rows) => [...rows, { slug: "", label: "", publishable: false }]);
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-5">
      <DialogHeader>
        ...
        <DialogDescription>
          {mode === "edit"
            ? "Update this platform’s identity, post types, and media/text limits."
      ...
      <Section title="Identity">
        <div className="grid gap-4">
          ...
          <Field
            label="Zernio slug"
            htmlFor="pf-slug"
            hint="The Zernio wire id (e.g. linkedin, twitter)."
          >
            <Input
              id="pf-slug"
              value={zernioId}
      ...
      {/* ── Guidance ─────────────────────────────────────────────── */}
      <Section title="Guidance" subtitle="Prose hints used by AI content generation.">
      ...
      >
        <div className="grid gap-2">
          ...
          <div>
            <Button type="button" variant="outline" size="sm" onClick={addPostType}>
          ...
          {tab === "Image" && (
            <div className="grid gap-4">
              <ByteField
                ...
                onChange={(b) => setImage((c) => ({ ...c, maxFileSizeBytes: b }))}
          ...
          {tab === "Video" && (
            <div className="grid gap-4">
              <ByteField
                ...
                onChange={(b) => setVideo((c) => ({ ...c, maxFileSizeBytes: b }))}
            ...
            <div className="grid gap-4">
              <div className="grid gap-4">
                <NumberField
                  ...
                  onChange={(n) => setText((c) => ({ ...c, maxContentChars: n }))}
                />
                <NumberField
              ...
              <div className="grid gap-2">
                ...
                {postTypes.filter((r) => r.slug.trim()).length === 0 ? (
                  <p className="text-xs text-tertiary-foreground">
                    Add post types above to set per-type character limits.
                  </p>
                ) : (
                  <div className="grid gap-2">
                    {postTypes
...
// Section is a titled group within the form, with a hairline divider heading.
function Section({
  title,
  subtitle,
  children,
